 ...
 * Component: TMS0
 * Component Simulink Path: TMS0
 * Simulation Mode: SIL
 *
 */

#define S_FUNCTION_NAME                TMS0_sbs
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString("SIL_TMS");
      mxArray *rhs[3];
      const char * simulinkBlockPath = ssGetPath(S);
      rhs[ 0 ] = mxCreateString(
        "@coder.connectivity.SimulinkInterface.getSILPILInterface");
...
static void mdlSetupRuntimeResources(SimStruct *S)
{
...

  {
    ...
    if (error != NULL) {
      ...
      ssSetErrorStatus( S,
                       "Failed to determine the installed In-the-Loop version for comparison against the In-the-Loop s-function version (release 23.2 (R2023b)_15). To avoid this error, remove the In-the-Loop s-function from your MATLAB path (e.g. delete it or move to a clean working directory).");
      return;
    }                                  /* if */
    ...
    if (strcmp(installVersion, "23.2 (R2023b)_15") != 0) {
      ssSetErrorStatus( S,
                       "The In-the-Loop s-function is incompatible with the installed In-the-Loop version (see ver('matlab')); it was generated for release 23.2 (R2023b)_15. To avoid this error, remove the In-the-Loop s-function from your MATLAB path (e.g. delete it or move to a clean working directory)");
      return;
    }                                  /* if */
